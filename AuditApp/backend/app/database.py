...
def run_migrations():
    ...
    if "spa_observations" in tables:
        existing_cols = {c["name"] for c in inspector.get_columns("spa_observations")}
        if "observation_type" not in existing_cols:
            with engine.begin() as conn:
                conn.execute(text("ALTER TABLE spa_observations ADD COLUMN observation_type VARCHAR"))
        with engine.begin() as conn:
            conn.execute(text("UPDATE spa_observations SET observation_type = 'Unannounced' WHERE observation_type IS NULL"))
