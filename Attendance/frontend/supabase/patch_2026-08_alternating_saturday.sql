-- Run in Project A's SQL editor. Adds alternating-Saturday support (e.g.
-- 1st & 3rd Saturday off, 2nd & 4th working) - a weekday can now have more
-- than one staff_schedule row, each scoped to specific occurrences of that
-- weekday within the month via the new week_occurrence column. null (the
-- default, and the only mode Sun-Fri will ever use) means "every occurrence",
-- unchanged from before.

alter table staff_schedule add column if not exists week_occurrence int[];

-- Guessing Postgres's auto-generated name for the old 3-column constraint -
-- harmless if the guess is wrong, "if exists" just no-ops. The replacement
-- constraint below is added unnamed (Postgres names it itself), so this
-- doesn't depend on the guess being right.
alter table staff_schedule drop constraint if exists staff_schedule_staff_id_weekday_effective_from_key;
alter table staff_schedule add unique (staff_id, weekday, effective_from, week_occurrence);
